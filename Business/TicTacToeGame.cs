...
namespace TicTacToe.Business
{
    ...
    public class TicTacToeGame : GameBase
    {
        ...
        {
            // Собираем четыре строки: 
            // 1) текущее состояние доски, 
            // 2) кто сейчас ходит, 
            // 3) сколько ходов сделано, 
            // 4) статус игры.
            string boardLine = new string(board);
            string currentPlayerLine = currentPlayer.ToString();
            string moveCountLine = moveCount.ToString();
            string statusLine = Status.ToString();

            // Записываем массив строк в файл за один вызов
